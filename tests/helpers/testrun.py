...
def run_text_to_db_similar_tests(expectIncident, docsJsonPath):
...
# inputs are expectIncident number and a ./tests/unit/testing_materials/*.json file path
#   (runs w/o redirecting output and err to files)
def run_embed_to_db_similar_tests(expectIncident, docsJsonPath):
    # Define command for subprocess
    cmd = ['sam', 'local', 'invoke', 'embed-to-db-similar', '-t', './cdk.out/AiidNlpLambdaStack.template.json', '-e', docsJsonPath]
